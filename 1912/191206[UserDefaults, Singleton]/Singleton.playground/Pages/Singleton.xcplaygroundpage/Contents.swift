//: [Previous](@previous)
import UIKit
/*:
 # Singleton
 - 특정 클래스의 인스턴스에 접근할 때 항상 동일한 인스턴스만을 반환하도록 하는 설계 패턴
 - 한 번 생성된 이후에는 프로그램이 종료될 때까지 항상 메모리에 상주
 - 어플리케이션에서 유일하게 하나만 필요한 객체에 사용
 - UIApplication, AppDelegate 등
 */
//항상 데이터를 똑같이사용할때 싱글톤을 사용

// 자기자신을 렛으로 선언하고 스테틱을 붙이는게 싱글톤클래스 형식이된다.

/*
 iOS 싱글톤 사용 예
 */
let screen = UIScreen.main
let userDefaults = UserDefaults.standard
let application = UIApplication.shared
let fileManager = FileManager.default
let notification = NotificationCenter.default


/*:
 ## Syntax
 */
//: ### Obj-C
//: ![objc_singleton](objc_singleton.png)
/*:
 ### Swift
 */

class NormalClass {
  var x = 0
}

let someObject1 = NormalClass()
someObject1.x = 5

let someObject2 = NormalClass()
someObject2.x = 1

let someObject3 = NormalClass()
someObject3.x = 10

someObject1.x
someObject2.x
someObject3.x



/*
 static 전역 변수로 선언한 것은 지연(lazy) 생성되므로
 처음 Singleton을 생성하기 전까지 메모리에 올라가지 않음
 */


//static let,var = 타입 프로퍼티: 실제로사용될때 만들어짐(지연생성)
//클래스자체에서 접근할수있는것 - static
class SingletonClass {
  static let shared = SingletonClass()
  var x = 0
}
// 자기타입자체를 클래스내부에서 자기자신을 shared에 넣어줌


let singleton1 = SingletonClass.shared // = let singleton1 = SingletonClass() 같은것
singleton1.x = 10

let singleton2 = SingletonClass.shared // 두번째 쉐어드는 이미만들어져있는 singleton1의 스텍의값을 2에 넣어주는것
singleton2.x = 20
// singleton1 = singleton2 같은의미라는것..
// 서로다른변수지만 따로따로 값을 설정해줬지만 같은놈이다

let someVar1 = NormalClass()
someVar1.x = 10
let someVar2 = someVar1
someVar2.x = 20

someVar1.x
someVar2.x
// 위의 스테틱으로 만든 클래스와 같은것 값이같음!

singleton1.x   //
singleton2.x   //

SingletonClass.shared.x = 30
// 다같은놈들에게 접근하게되어 하나만바꿔도 값이 공유가됨
SingletonClass.shared.x  //
singleton1.x  //
singleton2.x  //
SingletonClass.shared.x = 50
SingletonClass.shared.x
singleton1.x


SingletonClass().x = 99 // 만들고 변수를 따로 저장하지않아서 만들자마자 소멸
SingletonClass().x   // ()생성자를 새롭게만들어서 엑스를 보면 0을찍고 메모리에서 종료되고 사라짐
singleton1.x  //
singleton2.x  //



/*
 Q.
 항상 하나의 객체만을 사용하는 것을 보장해야 하는 상황에서
 아래와 같은 싱글톤 클래스를 만들었는데, 현재 상태에서 생길 수 있는 문제점은?
 */
//계속 인스턴스를 생성할수있는 가능성이있는데 하나의객체만을 사용해야하는것을 보장해야하는상황에서 해결할수있는 방법이란
// 생성자에 프라이빗을 적용하면 또 만드는걸 막을수있다.

class MySingleton {
  static let shared = MySingleton()
  var x = 0
    
  // 이렇게함으로써 새로운 생성을 막을수있음 공유만 가능해지는 객체로변신~!
//    private init() {
//
//    }
}

let object1 = MySingleton.shared

// 여전히 새로운 객체를 만들고 다른 객체에 접근 가능

let object2 = MySingleton()
object1.x = 10
object2.x = 20

object1.x
object2.x




/*
 외부에서 인스턴스를 직접 생성하지 못하도록 강제해야 할 경우 생성자를 private 으로 선언
 단, 일부러 새로운 것을 만들어서 쓸 수 있는 여지를 주고 싶은 경우는 무관
 */

class PrivateSingleton {
  static let shared = PrivateSingleton()
  
  // init을 private시켰으므로 쉐어드를 이용해서만 접근이가능. 고로 모두다 공유됨
  private init() {}
  var x = 1
}

//let uniqueSingleton = PrivateSingleton.init()
let uniqueSingleton1 = PrivateSingleton.shared
let uniqueSingleton2 = PrivateSingleton.shared
uniqueSingleton1.x
uniqueSingleton2.x

uniqueSingleton1.x = 20

uniqueSingleton1.x
uniqueSingleton2.x




//: [Next](@next)
